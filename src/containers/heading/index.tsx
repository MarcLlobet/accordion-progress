...
const getPercentage = ({
  ...
  const valueCheckedTasks = Object.entries(taskCheckedById)
      .reduce((prev, [taskId, checkedTask]) => 
        prev + (checkedTask ? taskValueById[taskId] : 0), 0)
...
const {
  ...
  const percentage = useMemo(() => getPercentage({
    taskCheckedById,
    taskValueById,
    totalTasksValue
  ...
  return (
    <Header>
      <Text size='big' as='h4'>Accordion</Text>
